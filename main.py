...
def initial_file_check():
    ...
    if isinstance(INTEGER_NUMS, str) or INTEGER_NUMS < 2:  # Need at least two numbers to multiply.
        print("FileError: Reverting integer_nums to defaults due to invalid values.")
        config['Settings']['integer_nums'] = '2'
        INTEGER_NUMS = 2

    if isinstance(PROBLEM_NUMS, str) or PROBLEM_NUMS < 1:  # Need at least one problem to solve.
...
def settings_menu():
    ...
    while True:
        ...
        if len(user_input) == 1 and user_input[0] == "help":
            ...
            if user_input[1] < 2:
                print("Error: Numbers of integers cannot be set below two.")  # Need two numbers to multiply...
            else:
...
# Play, interact (settings), or leave
def main():
    while True:
